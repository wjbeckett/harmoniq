...
async def get_pending_recommendations(
    ...
    try:
        if not recommendation_manager:
            raise HTTPException(status_code=500, detail="Recommendation manager not initialized")
...
async def get_all_recommendations(
    ...
    try:
        ...
        if search:
            recommendations = recommendation_manager.search_recommendations(search, status_filter)[:limit]
        else:
            recommendations = recommendation_manager.get_recommendations_by_status(status_filter, limit)
...
async def bulk_update_recommendations(request: BulkUpdateRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to bulk update recommendations")
...
async def process_approved_recommendations() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to process approved recommendations")
...
async def get_recommendation_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        approval_rate = (stats.get("total_approved", 0) / total_decisions * 100) if total_decisions > 0 else 0
...
async def cleanup_old_recommendations(days_old: int = Query(30, ge=1, le=365)) -> Dict[str, Any]:
...
async def get_album_preview(album_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        preview_data = {
            ...
            "musicbrainz_url": f"https://musicbrainz.org/release/{recommendation['mbid']}" if recommendation.get('mbid') else None
        }
