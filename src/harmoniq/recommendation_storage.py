...
class AlbumRecommendationManager:
    ...
    def __init__(self, config_dir: str = "/app/config"):
        ...
        self.recommendations_file = os.path.join(
            config_dir, "album_recommendations.json"
        )
        self.lock = Lock()
    ...
    def add_recommendation(self, album_data: Dict[str, Any]) -> str:
        ...
        with self.lock:
            ...
            album_id = (
                album_data.get("mbid")
                or f"album_{hash(f'{artist}_{title}')}_{int(datetime.now().timestamp())}"
            )
            ...
            recommendation = {
                ...
                "source": album_data.get(
                    "source", "library_grower"
                ...
                "external_ratings": album_data.get(
                    "external_ratings", {}
                ),  # LastFM, AllMusic, etc.
    ...
    def update_recommendation_status(
        ...
        with self.lock:
            ...
            self.recommendations["recommendations"][album_id][
                "user_decision_date"
            ] = datetime.now().isoformat()
    ...
    def get_recommendations_by_status(
        self, status: RecommendationStatus = None, limit: int = None
    ) -> List[Dict[str, Any]]:
        ...
        if status:
            recommendations = [
                r for r in recommendations if r["status"] == status.value
            ]
    ...
    def bulk_update_status(
        self, album_ids: List[str], status: RecommendationStatus, user_notes: str = ""
    ) -> Dict[str, bool]:
        """Update multiple recommendations at once."""
        results = {}
        for album_id in album_ids:
            results[album_id] = self.update_recommendation_status(
                album_id, status, user_notes
            )
        return results
    ...
    def cleanup_old_recommendations(self, days_old: int = 30):
        ...
        with self.lock:
            ...
            for album_id, rec in self.recommendations["recommendations"].items():
                if rec["status"] in [
                    RecommendationStatus.DENIED.value,
    ...
    def search_recommendations(
        self, query: str, status: RecommendationStatus = None
    ) -> List[Dict[str, Any]]:
        ...
        for rec in self.recommendations["recommendations"].values():
            ...
            if (
                ...
                or query_lower in rec["artist"].lower()
            ):
                results.append(rec)
