...
DEFAULT_PERIOD_VIBES = {
    "EarlyMorning": {
        "moods": ["Calm", "Peaceful", "Relaxed"],
        "styles": ["Ambient", "Acoustic", "Instrumental"],
    },
    "Morning": {
        "moods": ["Focused", "Energetic", "Upbeat"],
        "styles": ["Electronic", "Pop", "Rock"],
    },
    "Midday": {
    ...
    "Afternoon": {
        "moods": ["Energetic", "Rowdy", "Driving"],
        "styles": ["Rock", "Electronic", "Rap"],
    },
    "Evening": {
        "moods": ["Relaxed", "Cool", "Sentimental", "Romantic"],
        "styles": ["Jazz", "Blues", "Soul", "R&B"],
    },
    "LateNight": {
        "moods": ["Atmospheric", "Mellow", "Brooding", "Nocturnal"],
        "styles": ["Ambient", "Electronic", "Trip Hop", "Classical"],
    },
    "DefaultVibe": {"moods": ["Eclectic"], "styles": ["Mixed"]},
}
DEFAULT_COVER_PERIOD_COLORS = {  # Moved here for clarity
    "EarlyMorning": ((60, 70, 100), (100, 120, 160)),
    "Morning": ((100, 150, 200), (180, 210, 230)),
    "Midday": ((255, 200, 100), (255, 160, 80)),
    "Afternoon": ((255, 120, 80), (220, 80, 60)),
    "Evening": ((80, 60, 110), (140, 100, 160)),
    "LateNight": ((30, 30, 60), (70, 70, 100)),
...
try:
    ...
    ALL_CONFIG_DEFINITIONS = {
        ...
        "PLEX_TOKEN": {
            ...
            "type": str,
            "required": True,
            "yaml_path": "plex_token",
        },
        "PLEX_MUSIC_LIBRARY_NAMES": {
            "default": "Music",
            "type": list,
            "yaml_path": "plex_music_library_names",
        },  # Env var is comma-separated string
        ...
        "ENABLE_TIME_PLAYLIST": {
            "default": True,
            "type": bool,
            "yaml_path": "features.enable_time_playlist",
        },
        "ENABLE_PLAYLIST_COVERS": {
            "default": True,
            "type": bool,
            "yaml_path": "features.enable_playlist_covers",
        },
        "TIME_PLAYLIST_LEARN_FROM_HISTORY": {
            "default": True,
            "type": bool,
            "yaml_path": "features.time_playlist.learn_from_history",
        },
        "TIME_PLAYLIST_INCLUDE_HISTORY_TRACKS": {
            "default": True,
            "type": bool,
            "yaml_path": "features.time_playlist.include_history_tracks",
        },
        "TIME_PLAYLIST_USE_SONIC_EXPANSION": {
            "default": True,
            "type": bool,
            "yaml_path": "features.time_playlist.use_sonic_expansion",
        },
        "TIME_PLAYLIST_USE_SONIC_ADVENTURE": {
            "default": False,
            "type": bool,
            "yaml_path": "features.time_playlist.use_sonic_adventure",
        },
        "TIME_PLAYLIST_SONIC_SORT": {
            "default": True,
            "type": bool,
            "yaml_path": "features.time_playlist.sonic_sort",
        },
        "PLAYLIST_NAME_TIME": {
            "default": "Harmoniq Flow",
            "type": str,
            "yaml_path": "playlists.time_flow.name",
        },
        "PLAYLIST_SIZE_TIME": {
            "default": 50,
            "type": int,
            "yaml_path": "playlists.time_flow.size",
        },
        "TIME_PLAYLIST_MIN_RATING": {
            "default": 3,
            "type": int,
            "yaml_path": "playlists.time_flow.min_rating",
        },
        "TIME_PLAYLIST_EXCLUDE_PLAYED_DAYS": {
            "default": 21,
            "type": int,
            "yaml_path": "playlists.time_flow.exclude_played_days",
        },
        "TIME_PLAYLIST_MAX_SKIP_COUNT": {
            "default": 3,
            "type": int,
            "yaml_path": "playlists.time_flow.max_skip_count",
        },
        "TIME_PLAYLIST_HISTORY_LOOKBACK_DAYS": {
            "default": 90,
            "type": int,
            "yaml_path": "playlists.time_flow.history_lookback_days",
        },
        "TIME_PLAYLIST_HISTORY_MIN_PLAYS": {
            "default": 3,
            "type": int,
            "yaml_path": "playlists.time_flow.history_min_plays",
        },
        "TIME_PLAYLIST_HISTORY_MIN_RATING": {
            "default": 3,
            "type": int,
            "yaml_path": "playlists.time_flow.history_min_rating",
        },
        "TIME_PLAYLIST_TARGET_HISTORY_COUNT": {
            "default": 7,
            "type": int,
            "yaml_path": "playlists.time_flow.target_history_count",
        },
        "TIME_PLAYLIST_VIBE_ANCHOR_COUNT": {
            "default": 5,
            "type": int,
            "yaml_path": "playlists.time_flow.vibe_anchor_count",
        },
        "TIME_PLAYLIST_SONIC_SEED_TRACKS": {
            "default": 3,
            "type": int,
            "yaml_path": "playlists.time_flow.sonic_seed_tracks",
        },
        "TIME_PLAYLIST_SIMILAR_TRACKS_PER_SEED": {
            "default": 5,
            "type": int,
            "yaml_path": "playlists.time_flow.similar_tracks_per_seed",
        },
        "TIME_PLAYLIST_SONIC_MAX_DISTANCE": {
            "default": 0.45,
            "type": float,
            "yaml_path": "playlists.time_flow.sonic_max_distance",
        },
        "TIME_PLAYLIST_FINAL_MIX_RATIO": {
            "default": 0.4,
            "type": float,
            "yaml_path": "playlists.time_flow.final_mix_ratio",
        },
        "TIME_PLAYLIST_SONIC_SORT_SIMILARITY_LIMIT": {
            "default": 20,
            "type": int,
            "yaml_path": "playlists.time_flow.sonic_sort_similarity_limit",
        },
        "TIME_PLAYLIST_SONIC_SORT_MAX_DISTANCE": {
            "default": 0.65,
            "type": float,
            "yaml_path": "playlists.time_flow.sonic_sort_max_distance",
        },
        "TIME_PLAYLIST_LEARNED_VIBE_LOOKBACK_DAYS": {
            "default": 60,
            "type": int,
            "yaml_path": "playlists.time_flow.learned_vibe.lookback_days",
        },
        "TIME_PLAYLIST_LEARNED_VIBE_TOP_N_MOODS": {
            "default": 3,
            "type": int,
            "yaml_path": "playlists.time_flow.learned_vibe.top_n_moods",
        },
        "TIME_PLAYLIST_LEARNED_VIBE_TOP_M_STYLES": {
            "default": 3,
            "type": int,
            "yaml_path": "playlists.time_flow.learned_vibe.top_m_styles",
        },
        "TIME_PLAYLIST_LEARNED_VIBE_MIN_OCCURRENCES": {
            "default": 2,
            "type": int,
        ...
        "COVER_FONT_FILE_PATH": {
            ...
            "type": str,
            "yaml_path": "cover_settings.font_file_path",
        },
        "COVER_OUTPUT_PATH": {
            "default": "/tmp/harmoniq_cover.png",
            "type": str,
            "yaml_path": "cover_settings.output_path",
        },
        "CONFIG_FILE_PATH": {"default": "/app/config/config.yaml", "type": str},
        "LOG_LEVEL": {"default": "INFO", "type": str, "yaml_path": "log_level"},
        "TIME_PERIOD_SCHEDULE_RAW_ENV": {
            "default": "Morning=7;Midday=12;Afternoon=16;Evening=19;LateNight=22",
            "type": str,
            "env_only": True,
        },
    }
    ...
    _CONFIG_FILE_PATH = os.environ.get(
        "CONFIG_FILE_PATH", ALL_CONFIG_DEFINITIONS["CONFIG_FILE_PATH"]["default"]
    )
    yaml_config_data = {}
    if os.path.exists(_CONFIG_FILE_PATH):
        try:
            ...
            logger.info(
                f"Successfully loaded configuration from YAML file: {_CONFIG_FILE_PATH}"
            )
        except yaml.YAMLError as e:
            logger.error(f"Error parsing YAML config at {_CONFIG_FILE_PATH}: {e}.")
        except IOError as e:
            logger.warning(f"Could not read YAML config at {_CONFIG_FILE_PATH}: {e}.")
    else:
        logger.info(
            f"No YAML config file found at {_CONFIG_FILE_PATH}. Using defaults and environment variables."
        )
    ...
    def _get_config_value(env_key, definition, yaml_data):
        ...
        if env_val is not None:
            ...
            try:
                ...
                logger.error(f"Invalid ENV type for {env_key}. Defaulting.")
                return default_val
        ...
        if yaml_path and yaml_data:
            ...
            if yaml_val is not None:
                logger.debug(
                    f"Loading '{env_key}' from YAML '{yaml_path}' ('{yaml_val}')."
                )
                if val_type == bool:
                    return bool(yaml_val)
                if val_type == list:
                    return yaml_val if isinstance(yaml_val, list) else default_val
                try:
                    return val_type(yaml_val)
                except ValueError:
                    logger.error(
                        f"Invalid YAML type for {env_key} at {yaml_path}. Defaulting."
                    )
                    return default_val
        logger.debug(f"Loading '{env_key}' from default ('{default_val}').")
        return default_val
    ...
    logger.info(
        f"[CONFIG_PY_FINAL] TIMEZONE is set to: '{TIMEZONE}' (Type: {type(TIMEZONE)})"
    )

    # --- Special Handling for COVER_PERIOD_COLORS ---
    COVER_PERIOD_COLORS = DEFAULT_COVER_PERIOD_COLORS.copy()
    yaml_cover_settings_colors = _get_nested_val(
        yaml_config_data, "cover_settings.period_colors"
    )
    if isinstance(yaml_cover_settings_colors, dict):
        logger.info(
            "Loading COVER_PERIOD_COLORS from YAML 'cover_settings.period_colors'."
        )
        for period_name, color_values in yaml_cover_settings_colors.items():
            ...
            if isinstance(color_values, list) and len(color_values) == 2:
                try:
                    ...
                    if isinstance(color_values[0], str) and color_values[0].startswith(
                        "#"
                    ):
                        c1_tuple = tuple(
                            int(color_values[0].lstrip("#")[i : i + 2], 16)
                            for i in (0, 2, 4)
                        )
                    elif (
                        isinstance(color_values[0], list) and len(color_values[0]) == 3
                    ):
                        c1_tuple = tuple(color_values[0])
                    if isinstance(color_values[1], str) and color_values[1].startswith(
                        "#"
                    ):
                        c2_tuple = tuple(
                            int(color_values[1].lstrip("#")[i : i + 2], 16)
                            for i in (0, 2, 4)
                        )
                    elif (
                        isinstance(color_values[1], list) and len(color_values[1]) == 3
                    ):
                        c2_tuple = tuple(color_values[1])
                    if (
                        c1_tuple
                        and c2_tuple
                        and all(0 <= c <= 255 for c in c1_tuple)
                        and all(0 <= c <= 255 for c in c2_tuple)
                    ):
                        COVER_PERIOD_COLORS[period_name] = (c1_tuple, c2_tuple)
                        logger.debug(
                            f"Overriding cover colors for '{period_name}' from YAML: {COVER_PERIOD_COLORS[period_name]}"
                        )
                    else:
                        logger.warning(
                            f"Invalid color format for period '{period_name}' in YAML."
                        )
                except Exception as e_color:
                    logger.warning(
                        f"Error parsing colors for '{period_name}' in YAML: {e_color}"
                    )
            else:
                logger.warning(
                    f"Invalid format for period '{period_name}' in YAML 'cover_settings.period_colors'."
                )
    elif yaml_cover_settings_colors is not None:
        logger.warning("'cover_settings.period_colors' in YAML is not a dict.")
    ...
    if globals().get("ENABLE_TIME_PLAYLIST"):
        yaml_time_periods = (
            yaml_config_data.get("time_periods")
            if isinstance(yaml_config_data.get("time_periods"), list)
            else []
        )
        ...
        if yaml_time_periods:
            ...
            for entry in yaml_time_periods:  # Iterate through list of dicts
                if (
                    ...
                    and "name" in entry
                    and "start_hour" in entry
                ):
                    name, hour = entry["name"], entry["start_hour"]
                    crit_yaml = entry.get("criteria", {})
                    yaml_m = crit_yaml.get("moods", [])
                    yaml_s = crit_yaml.get("styles", [])
                    default_vibe = DEFAULT_PERIOD_VIBES.get(
                        name, DEFAULT_PERIOD_VIBES["DefaultVibe"]
                    )
                    moods = [
                        m.strip().capitalize() for m in yaml_m if m
                    ] or default_vibe["moods"]
                    styles = [
                        s.strip().capitalize() for s in yaml_s if s
                    ] or default_vibe["styles"]
                    if name not in temp_periods_yaml:
                        temp_periods_yaml[name] = {
                            "hour": hour,
                            "moods": moods,
                            "styles": styles,
                        }
                        ...
                        logger.warning(
                            f"Duplicate YAML period name '{name}'. Ignoring subsequent."
                        )
            if temp_periods_yaml:
                sorted_names = sorted(
                    temp_periods_yaml.keys(), key=lambda k: temp_periods_yaml[k]["hour"]
                )
                for name_s in sorted_names:
                    SCHEDULED_PERIODS.append(
                        {
                            "name": name_s,
                            "start_hour": temp_periods_yaml[name_s]["hour"],
                            "criteria": {
                                "moods": temp_periods_yaml[name_s]["moods"],
                                "styles": temp_periods_yaml[name_s]["styles"],
                            },
                        }
                    )
                if SCHEDULED_PERIODS:
                    parsed_from_yaml = True
                    logger.info(f"Loaded {len(SCHEDULED_PERIODS)} periods from YAML.")

        if not parsed_from_yaml and globals().get("TIME_PERIOD_SCHEDULE_RAW_ENV"):
            logger.info(
                f"No periods from YAML or YAML not used. Parsing TIME_PERIOD_SCHEDULE_RAW_ENV: '{globals()['TIME_PERIOD_SCHEDULE_RAW_ENV']}'"
            )
            period_defs_env = [
                p.strip()
                for p in globals()["TIME_PERIOD_SCHEDULE_RAW_ENV"].strip().split(";")
                if p.strip()
            ]
            temp_periods_env = {}
            for p_def in period_defs_env:  # Existing ENV parsing logic
                ...
                try:
                    ...
                    # ... validation ...
                    user_moods_raw_env = os.environ.get(
                        f"TP_DEFINE_{name.upper()}_MOODS", ""
                    )
                    user_styles_raw_env = os.environ.get(
                        f"TP_DEFINE_{name.upper()}_STYLES", ""
                    )
                    default_vibe_env = DEFAULT_PERIOD_VIBES.get(
                        name, DEFAULT_PERIOD_VIBES["DefaultVibe"]
                    )
                    moods_env = (
                        [
                            m.strip().capitalize()
                            for m in user_moods_raw_env.split(",")
                            if m.strip()
                        ]
                        if user_moods_raw_env
                        else default_vibe_env["moods"]
                    )
                    styles_env = (
                        [
                            s.strip().capitalize()
                            for s in user_styles_raw_env.split(",")
                            if s.strip()
                        ]
                        if user_styles_raw_env
                        else default_vibe_env["styles"]
                    )
                    if name not in temp_periods_env:
                        temp_periods_env[name] = {
                            "hour": hour,
                            "moods": moods_env,
                            "styles": styles_env,
                        }
                except ValueError:
                    pass  # Logged by get_env_var or earlier
            if temp_periods_env:
                sorted_names_env = sorted(
                    temp_periods_env.keys(), key=lambda k: temp_periods_env[k]["hour"]
                )
                for name_s_env in sorted_names_env:
                    SCHEDULED_PERIODS.append(
                        {
                            "name": name_s_env,
                            "start_hour": temp_periods_env[name_s_env]["hour"],
                            "criteria": {
                                "moods": temp_periods_env[name_s_env]["moods"],
                                "styles": temp_periods_env[name_s_env]["styles"],
                            },
                        }
                    )
        ...
        if not SCHEDULED_PERIODS:
            logger.warning(
                "Harmoniq Flow enabled, but no valid periods parsed from YAML or ENV."
            )
    ...
    if globals().get("TIME_PLAYLIST_USE_SONIC_EXPANSION"):
        ...
        if not (0.0 <= mix_ratio <= 1.0):
            logger.warning(
                "TIME_PLAYLIST_FINAL_MIX_RATIO must be between 0.0 and 1.0. Defaulting to 0.5"
            )
            globals()["TIME_PLAYLIST_FINAL_MIX_RATIO"] = 0.5
        if globals().get("TIME_PLAYLIST_SONIC_SEED_TRACKS", 0) <= 0:
            logger.warning(
                "TIME_PLAYLIST_SONIC_SEED_TRACKS must be positive. Disabling sonic expansion."
            )
            globals()["TIME_PLAYLIST_USE_SONIC_EXPANSION"] = False
        if globals().get("TIME_PLAYLIST_SIMILAR_TRACKS_PER_SEED", 0) <= 0:
            logger.warning(
                "TIME_PLAYLIST_SIMILAR_TRACKS_PER_SEED must be positive. Disabling sonic expansion."
            )
            globals()["TIME_PLAYLIST_USE_SONIC_EXPANSION"] = False
    ...
    if globals().get("TIME_PLAYLIST_LEARN_FROM_HISTORY", False):
        if globals().get("TIME_PLAYLIST_LEARNED_VIBE_LOOKBACK_DAYS", 0) <= 0:
            logger.warning(
                "TIME_PLAYLIST_LEARNED_VIBE_LOOKBACK_DAYS must be positive. Disabling vibe learning."
            )
    ...
    if globals().get("ENABLE_PLAYLIST_COVERS"):
        ...
        if font_path == ALL_CONFIG_DEFINITIONS["COVER_FONT_FILE_PATH"][
            "default"
        ] and not os.path.exists(font_path):
            logger.warning(
                f"Playlist covers enabled, but default font at '{font_path}' might not exist. Consider bundling a font or setting COVER_FONT_FILE_PATH in config.yaml or ENV."
            )
    ...
    logger.critical(
        f"CRITICAL UNEXPECTED CONFIGURATION ERROR: {e_unexpected_config}", exc_info=True
    )
    raise SystemExit(
        f"Unexpected critical configuration loading error: {e_unexpected_config}"
    )
...
final_log_level_to_apply = str(
    globals().get("LOG_LEVEL", "INFO")
