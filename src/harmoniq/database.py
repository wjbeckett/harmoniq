...
class HarmoniqDatabase:
    ...
    def start_library_sync(self, sync_type: str) -> int:
        """
        Start tracking a library sync operation.

        Args:
            sync_type: Type of sync ('plex', 'lidarr', 'combined')
        ...
        try:
            ...
            cursor.execute(
                """
                INSERT INTO library_syncs (sync_type, status, started_at)
                VALUES (?, ?, ?)
            """,
                (
                    sync_type,
                    LibrarySyncStatus.STARTED.value,
            ...
            logger.debug(
                f"Started library sync tracking: ID {sync_id}, type {sync_type}"
            )
            return sync_id

        except Exception as e:
            logger.error(f"Error starting library sync tracking: {e}")
            return 0
    ...
    def complete_library_sync(
        ...
        Args:
            sync_id: Sync ID from start_library_sync
            status: Final sync status
            results: Dictionary with sync results and statistics
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(
            ...
            self.conn.commit()
            logger.debug(
                f"Completed library sync tracking: ID {sync_id}, status {status.value}"
            )

        except Exception as e:
            logger.error(f"Error completing library sync tracking: {e}")

    def get_recent_library_syncs(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent library sync operations."""
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                """
            ...
            for row in cursor.fetchall():
                ...
                if sync_data["results"]:
                    sync_data["results"] = json.loads(sync_data["results"])
                syncs.append(sync_data)

            return syncs

        except Exception as e:
            logger.error(f"Error getting recent library syncs: {e}")
            return []

    def get_library_sync_stats(self) -> Dict[str, Any]:
        """Get library sync statistics."""
        try:
            cursor = self.conn.cursor()

            # Get total syncs by status
            cursor.execute(
                """
            ...
            return {
                ...
                "successful_syncs": status_counts.get("success", 0)
            ...
            logger.error(f"Error getting library sync stats: {e}")
            return {}
